...
httpServer.listen(6001, (req, res) => {
    console.log("server 6001");
});
...
//mongoose.connect("mongodb://0.0.0.0:27017/jpludo", {
    ...
    if (!err) {
        console.log(`you are connected hiplayludo`);
    } else {
        console.log(`you are not connected hiplayludo`);
    }
})
...
wss.on('connection', function (ws) { 
    ws.on('message',async function (message) { 
        ...
        //console.log('even is =>',event,'data and 馃',data)
        switch (event) {
    case 'gameCreated': {
...
const interval = setInterval(function ping() {
    wss.clients.forEach(function each(ws) {
        //console.log('is terminate 馃',ws.isAlive === false,'adn sockt state 馃棏锔�',ws.readyState);
