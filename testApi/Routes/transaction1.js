...
router.post("/user/depositeupi", Auth, async (req, res) => {
  ...
  user_token: '6619e1da7dab0af2c73122b59ab49e14',
  amount: txn.amount.toString(),
  order_id: txn._id,
  redirect_url: 'https://haryanaludo.com/Return',
  remark1: 'Dharmendra',
  remark2: 'user desposit'
};

const formData = new URLSearchParams(data).toString();
    
    await axios.post('https://upig.in/api/create-order',formData,
          {
            headers: {
        ...
        if(result.data.status){
            ...
            txn.payment_gatway = req.body.payment_gatway;
            txn.order_id = txn._id;
            txn.order_token = result.data.result.orderId;
...
router.post('/depositupipay/response', Auth, async (req, res) => {
    ...
    if (txn.order_id === orderID && txn.order_token === orderToken && txn.status != 'PAID' ) {
        await axios.post('https://upig.in/api/check-order-status',formData,
          {
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded'
            }
...
//         {
//             "key": "b5bd44d8-2270-4f1f-b355-1f5402506367",
//             "client_txn_id": txn._id,
//             "amount": txn.amount.toString(),
//             "p_info": "Add Wallet",
//             "customer_name": "Dharmendra", //req.body.account_name,
//             "customer_email": "Dharmendra@gmail.com", //req.body.account_mail_id,
//             "customer_mobile": user.Phone.toString(),
//             "redirect_url": "https://ludopaisa.com/return",
//             "udf1": "user desposit"
//           },
//           {
//             headers: {
...
router.post("/withdraw/payoutmanualupi", Auth, async (req, res) => {
    ...
    try {
        ...
        if(user1.Wallet_balance==calculatedWallet)
        {
            ...
            if (((parseInt(user1.lastWitdrawl) + 9600000) < currentTime && lasttrans.status=='SUCCESS') || (user1.lastWitdrawl == null) || (!lasttrans || lasttrans.status!='SUCCESS')) {
                    if (amount <= 10000) {
...
router.post("/withdraw/request", Auth, async (req, res) => {
    try {
        ...
        if (pendingGame.length == 0) {
            if (((parseInt(user.lastWitdrawl) + 9600000) < currentTime && lasttrans.status=='SUCCESS') || (user.lastWitdrawl == null) || (!lasttrans || lasttrans.status!='SUCCESS')) {
                if (req.body.amount >= 190) {
                    ...
                    res.send({ "msg": "Withdrawal amount should be greater or equal to 190 Rupees", "success": false })
