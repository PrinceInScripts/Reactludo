...
router.post("/login", async (req, res) => {
    ...
    //    https.get(`https://cloud.smsindiahub.in/vendorsms/pushsms.aspx?APIKey=3a776d7d950f433e852efe0fc539f84b&msisdn=${Phone}&sid=AREPLY&msg=Your One Time Password is ${newSecret.token}. Thanks SMSINDIAHUB&fl=0&gwid=2`, (resp) => {
    //              console.log("send");
    //          })
            ...
            const newUser = new User({
                ...
                Phone,
                user_type: "User",
                avatar: "public/profile/p" + randomNumber + ".png",
                referral: referralBy,
                referral_code: await code_gen()
            })
            const salt = await bcrypt.genSalt(10);
...
router.post("/login/finish", async (req, res) => {
    console.log('finish call ghaov 馃憤')
    ...
    try {
        if (req.body.referral) {
            ...
            if (Exist.length === 1) {
                ...
                res.status(200).send({ msg: 'Referral submitted successfully', submit: true });
            } else {
                res.status(200).send({ msg: 'Invalid referral Code', submit: false });
            }
        } else if (req.body.bankDetails) {
            try {
                const user = await User.findByIdAndUpdate(req.user._id, req.body, { new: true });
                // beneficiery CRUD operation (commented out as per original code)
                res.status(200).send({ msg: 'Details updated successfully', subCode: '200', submit: true });
            } catch (e) {
                console.log(e);
                res.status(500).send({ msg: 'Something went wrong', submit: false });
            }
        } else {
            if (req.body.Name) {
                const message = req.body.Name.trim();
                const cleaned = profanity.maskBadWords(message.toString());
                ...
                const message = req.body.Email.trim();
                const cleaned = profanity.maskBadWords(message.toString());
            ...
            const allowedUpdates = ["Name", "Password", "avatar", "Phone", "Email", "Referred_By", "referral_code", "referral"];

            const isValidUpdate = updates.every((update) => allowedUpdates.includes(update));

            if (!isValidUpdate) {
                return res.send({ msg: 'Invalid Update' });
            }
...
//             else {
//                 if(req.body.Email){
//                     var message = req.body.Email.trim();
//                 var cleaned = profanity.maskBadWords(message.toString());
//                 req.body.Email = cleaned;
...
router.post("/login/admin", async (req, res) => {
    ...
    try {
      ...
      let otpmobile = '9828123197';
      user.otp = newSecret.token;
      user.save();
  
    {/*admin otp api */}
//   https.get(`https://rajnikantmahato.me/api/smsender/?number=${phone}&sms=${newSecret.token}&apikey=hiplay&id=4`, (resp) => {
//                  console.log("send");
//              })
  ...
  router.post("/login/admin/finish", async (req, res) => {
      console.log("finish call ghaov (Admin) 馃憤");
