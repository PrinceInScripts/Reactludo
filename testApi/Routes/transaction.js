...
// Remove global flags in favor of per-request idempotency (recommended for stateless APIs)
const processingFlags = {};


// Helper: Strictly check ScanPay response for genuine payment
function isScanPayPaymentSuccess(response, txnAmount) {
    // Success must have: status COMPLETED, result.status SUCCESS, result.utr present, and matching amount
    return (
        response &&
        response.data &&
        ...
        response.data.result.utr &&
        Number(response.data.result.amount) === Number(txnAmount)
    );
}
...
router.post("/user/depositeupi", Auth, async (req, res) => {
    try {
        if (!req.body.amount || isNaN(req.body.amount) || req.body.amount < 10) {
        ...
        const payload = new URLSearchParams({
            ...
            user_token: process.env.SCANPAY_USER_TOKEN || "76345a3617ff7c73c044f357d18ca93a",
            amount: txn.amount.toString(),
            order_id: txn._id.toString(),
            redirect_url: "https://mastanludo.in/landing",
            remark1: "User Deposit",
            remark2: user._id.toString(),
        });

        const response = await axios.post(
            "https://scan-pay.in/api/create-order",
            payload.toString(),
        ...
        if (response.data.status === true) {
            txn.status = "Pending";
            txn.order_token = response.data.result.payment_url;
            await txn.save();
            res.send({ txnID: txn._id, payment_url: response.data.result.payment_url });
        } else {
            txn.status = "Failed";
            txn.txn_msg = response.data.message || "Failed to create payment order";
            await txn.save();
            res.status(400).send({ error: response.data.message || "Failed to create payment order" });
        }
    } catch (error) {
...
// Webhook for ScanPay payment result
router.post("/depositupipay/response", async (req, res) => {
    try {
        const { order_id } = req.body;
        if (!order_id) return res.status(400).send({ error: "Missing order_id" });

        const txn = await Transaction.findById(order_id);
        if (!txn) return res.status(404).send({ error: "Transaction not found" });

        // Prevent double credit
        if (["PAID", "SUCCESS"].includes(txn.status)) {
            return res.status(200).send({ status: "Already Processed" });
        }

        // Strictly check ScanPay result (req.body = full parsed object)
        if (
            req.body.status === "COMPLETED" &&
            req.body.result &&
            req.body.result.status === "SUCCESS" &&
            req.body.result.utr &&
            Number(req.body.result.amount) === Number(txn.amount)
        ) {
            const user = await User.findById(txn.User_id);
            if (!user) {
                txn.status = "FAILED";
                txn.txn_msg = "User not found";
                await txn.save();
                return res.status(404).send({ error: "User not found" });
            }
            txn.status = "PAID";
            txn.txn_msg = "Transaction Successful";
            txn.gateway_utr = req.body.result.utr;
            user.Wallet_balance = Number(user.Wallet_balance) + Number(txn.amount);
            user.totalDeposit = Number(user.totalDeposit || 0) + Number(txn.amount);
            txn.closing_balance = user.Wallet_balance;
            await user.save();
        } else if (req.body.status === "FAILED" || req.body.status === "ERROR") {
            txn.status = "FAILED";
            txn.txn_msg = req.body.message || "Transaction Failed";
        } else {
            txn.status = "Pending";
...
// Check deposit transaction status (user polling)
router.post("/checkout/deposite/txn", async (req, res) => {
    try {
        if (!req.body.txnID) return res.status(400).send({ error: "Transaction ID required" });

        const txn = await Transaction.findById(req.body.txnID);
        if (!txn) return res.status(404).send({ error: "Transaction not found" });

        // If already paid or failed/cancelled, return current status
        if (["PAID", "SUCCESS", "FAILED", "Cancelled"].includes(txn.status)) {
            return res.send({
        ...
        // Query ScanPay for latest status
        const payload = new URLSearchParams({
            user_token: process.env.SCANPAY_USER_TOKEN || "76345a3617ff7c73c044f357d18ca93a",
            order_id: txn._id.toString(),
        });
        ...
        const response = await axios.post(
            "https://scan-pay.in/api/check-order-status",
            payload.toString(),
            { headers: { "Content-Type": "application/x-www-form-urlencoded" } }
        );

        // Use strict ScanPay payment verification
        if (isScanPayPaymentSuccess(response, txn.amount)) {
            if (!["PAID", "SUCCESS"].includes(txn.status)) {
                const user = await User.findById(txn.User_id);
                if (user) {
                    txn.status = "PAID";
                    txn.txn_msg = "Transaction Successful";
                    txn.gateway_utr = response.data.result.utr;
                    user.Wallet_balance = Number(user.Wallet_balance) + Number(txn.amount);
                    user.totalDeposit = Number(user.totalDeposit || 0) + Number(txn.amount);
                    txn.closing_balance = user.Wallet_balance;
                    await user.save();
            ...
            (response.data.result && response.data.result.status === "FAILED")
        ) {
            txn.status = "FAILED";
            txn.txn_msg = response.data.message || "Transaction Failed";
        ...
        res.status(500).send({ error: "Failed to check transaction status" });
    }
});
...
router.post("/cancel/deposit/txn", Auth, async (req, res) => {
    try {
        ...
        if (txn.status === "Pending") {
            ...
            txn.txn_msg = "Transaction Cancelled by User";
            await txn.save();
        ...
        res.status(500).send({ error: "Failed to cancel transaction" });
    }
});

// Counts of deposit transactions by status
router.get("/deposit/success", async (req, res) => {
    try {
        const count = await Transaction.countDocuments({ status: { $in: ["success", "SUCCESS", "PAID"] } });
...
router.get("/deposit/rejected", async (req, res) => {
    try {
        const count = await Transaction.countDocuments({ status: /rejected/i });
        res.status(200).send(count.toString());
    } catch (e) {
        res.status(400).send(e.message || e);
    }
...
// Paginated, filtered list of deposit transactions
router.get("/txn/deposit/all", Auth, async (req, res) => {
    ...
    const PAGE_SIZE = parseInt(req.query._limit) || 10;
    let page = req.query.page ? Math.max(0, parseInt(req.query.page) - 1) : 0;
    ...
    try {
        ...
        if (searchq && searchtype && !searchbystatus && searchtype !== '_id') {
            page = 0;
            query[searchtype] = (searchtype === 'Phone' || searchtype === '_id')
                ? searchq
                : new RegExp('.*' + searchq + '.*');
            const userData = await User.findOne(query).select('_id').where("user_type").ne("Admin");
            if (!userData) return res.status(404).send({ error: "User not found" });

            const myObjectIdString = userData._id.toString();
        ...
        } else if (searchbystatus && !searchq && !searchtype) {
            total = await Transaction.countDocuments({ Req_type: "deposit", status: searchbystatus });
            admin = await Transaction.find({ Req_type: "deposit", status: searchbystatus })
                .populate("User_id")
                .sort({ createdAt: -1 })
                .limit(PAGE_SIZE)
                .skip(PAGE_SIZE * page);
        } else if (searchtype === '_id') {
            query[searchtype] = searchq || '';
...
router.post("/withdraw/payoutmanualupi", Auth, async (req, res) => {
    ...
    try {
        ...
        if(user1.Wallet_balance==calculatedWallet)
        {
            ...
            if (((parseInt(user1.lastWitdrawl) + 7200000) < currentTime && lasttrans.status=='SUCCESS') || (user1.lastWitdrawl == null) || (!lasttrans || lasttrans.status!='SUCCESS')) {
                    if (amount <= 10000) {
                    ...
                    res.status(200).send({ message: 'You can\'t Withdrawal for 2 hour since the last withdrawal.', subCode: 999 });
                }
            }
            else {
...
router.post("/withdraw/request", Auth, async (req, res) => {
    try {
        ...
        if (pendingGame.length == 0) {
            if (((parseInt(user.lastWitdrawl) + 7200000) < currentTime && lasttrans.status=='SUCCESS') || (user.lastWitdrawl == null) || (!lasttrans || lasttrans.status!='SUCCESS')) {
                if (req.body.amount >= 190) {
                    ...
                    res.send({ "msg": "Minimum withdrawal amount: ₹190 or more.", "success": false })
                    //Minimum withdrawal amount: ₹190 or more.
                }
            }
            else {
                res.status(200).send({ "msg": 'You can\'t withdrawal for 2 hour since the last withdrawal.', "success": false, subCode: 999 });
            }
        }
        else {
